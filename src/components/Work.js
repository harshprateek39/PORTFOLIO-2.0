  ...
  return (
    <AnimatePresence>
      <motion.div ref={ref2}  style={{scaleX}}
        className="  min-h-screen md:pl-24 grid grid-flow-row md:grid-cols-7 gap-1 grid-cols-1 md:pr-24 md:pt-14 md:pb-14 overflow-visible "
        id="project"
      >
        <motion.div initial={{ x:-10}} whileInView={{ x:0}} transition={{ type:"spring", duration:0.5, delay:0.2}} className=" flex flex-col  overflow-visible  col-span-3  ">
          <div className=" flex justify-start items-baseline gap-4 pb-8  ">
            <motion.div
              ...
              viewport={{ once: true }}
              className="  whitespace-nowrap text-slate-200 font-semibold text-3xl"
            >
              Experience
            </motion.div>
            <motion.div
   ...
   {/* project section */}
        ...
        <motion.div initial={{ x:10}} whileInView={{ x:0}} transition={{ type:"spring", duration:0.5, delay:0.2}} className=" md:col-span-4 ">
          <div className=" flex justify-end items-baseline gap-4 pl-4 ">
          <motion.div
              initial={{ scaleX: 0 }}
              whileInView={{ scaleX: 1 }}
              viewport={{ once: true }}
          ...
          <motion.div initial={ { scrollBy:0 }} id="project1"  whileInView={{ scrollBy:300}}   className=" scroll-smooth flex flex-col snap-y gap-2 overflow-y-scroll max-h-[80vh] pl-4  " 
          transition= {{
              type: "spring",
              bounce: 0,
              duration: 0.7,
          ...
          {data3.map((data,index)=>{return (
            <motion.div  key={index} initial={{ opacity:0,  y:20 ,scaleX:0.7}}  onMouseEnter={()=>setProjectIndex(index)} onMouseLeave={()=>setProjectIndex(-1)}   whileInView={{opacity:1 ,y:0 ,scaleX:1 } } transition={{duration:0.5, type:"spring"}} className=" drop-shadow-2xl shadow-inner px-2 shadow-blue-600/25  bg-black/20 rounded-md flex flex-col">
              ...
              <div className=" grid grid-flow-col  grid-cols-5 ">
                <div className="  col-span-3">{data.description}</div>
                 <motion.div className=" bg-red-900 rounded-md col-span-2 overflow-hidden"><motion.img initial={{ scale:1 }} whileHover={{scale:1.2,  rotateZ:5 }} className=" rounded-md" src={data.image}></motion.img></motion.div> 
              </div>
